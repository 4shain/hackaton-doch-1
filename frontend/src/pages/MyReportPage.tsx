...
export default function MyReportPage() {
  ...
  const load = useCallback(() => {
    setLoadError(null)
    api.myReports(today, today).then(setReports, (e) => setLoadError(errorMessage(e)))
  }, [today])
  useEffect(load, [load])
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Box>
            ...
            <Typography sx={{ opacity: 0.9, fontSize: 14 }}>
              {current ? 'אפשר לעדכן את הדיווח של היום. שינוי דיווח מאושר יחזיר אותו לאישור המפקד.' : 'דיווח נוכחות להיום'}
            </Typography>
          </Box>
        </Stack>
